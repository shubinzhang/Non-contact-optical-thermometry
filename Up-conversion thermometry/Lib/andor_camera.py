...
class AndorCamera(): #
    ...
    def __init__(self):
        ...
        if platform.system() == "Linux":
            ...
            dllname = "C:\\Program Files\\Andor SDK\\atmcd32d"
...
if __name__ == "__main__":
	...
	data = []
	camera = AndorCamera()
	print "Ser number = ", camera.GetCameraSerialNumber()
	camera.SetCoolerMode(1)
	camera.CoolerON()
	camera.SetTemperature(-85)
	temp = camera.GetTemperature()
	print camera.GetTemperature()
	#camera.SetAcquisitionMode(1)
	#camera.SetExposureTime(5)
	#camera.SetReadMode(4)
	#camera.SetShutter(1, 0, 5000, 5000)
	#camera.StartAcquisition()
	while temp>(-85):
		camera.GetTemperature()
		temp = float(camera._temperature)
		print "Current temperature = ", temp
		sleep(1)
	print "Camera is cooled down to:", temp
	
	print "GetNumberADChannels ", camera.GetNumberADChannels()
	print "GetNumberPreAmpGains ", camera.GetNumberPreAmpGains() 
	print "GetNumberVSSpeeds = ", camera.GetNumberVSSpeeds()
	print "GetNumberHSSpeeds = ", camera.GetNumberHSSpeeds()
	print "GetHSSpeed" , camera.GetHSSpeed()
	print "GetPreAmpGain ", camera.GetPreAmpGain()
	print "GetVSSpeed ", camera.GetVSSpeed()
	...
	print "GetPreAmpGain ", camera.GetPreAmpGain()
	print "GetVSSpeed ", camera.GetVSSpeed()
	camera.SetPreAmpGain(0)
	camera.SetVSSpeed(0)
	
	camera.SetADChannel(1)
	print "GetNumberADChannels ", camera.GetNumberADChannels()
	print "GetNumberPreAmpGains ", camera.GetNumberPreAmpGains() 
	print "GetNumberVSSpeeds = ", camera.GetNumberVSSpeeds()
	print "GetNumberHSSpeeds = ", camera.GetNumberHSSpeeds()
	...
	camera.SetAcquisitionMode(3) # 1 - single scan
	camera.SetNumberAccumulations(1)
	camera.SetReadMode(4) # 0 - FVB, 3 - single track
	camera.SetNumberKinetics(2)
	camera.SetExposureTime(0.01)
	camera.SetTriggerMode(0)
	print "camera._width = ", camera._width
	print "camera._height = ", camera._height
	camera.GetAcquisitionTimings()
	camera.StartAcquisition()
	slptm = float(camera._kinetic)
	print "sleeptime = ", slptm
	sleep(slptm+0.5)
	data = camera.GetAcquiredData(data)
	data = resize(data, (1024,1024))
	camera.SaveAsFITS("test.fits",0)
